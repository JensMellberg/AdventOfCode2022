...
{
    public class Problem26 : StringProblem
    {
        ...
        public override void Solve(IEnumerable<string> testInput)
        {
            ...
            var numberPad2 = this.CreateNumberPad(10);
        ...
        private class KeyPad
        {
            ...
            {
                var presses = "";
                (int x, int y) pointerStart = (pointerPos.x, pointerPos.y);
                for (var i = 0; i < result.Length; i++)
                {
                    var c = result[i];
                    (int x, int y) newPoint = (0, 0);
                    var localresult = c.ToString();
                    var previousResult = "";
                    var pointer = i;
                    while (ResultDict.ContainsKey((pointerPos.x, pointerPos.y, localresult)))
                    {
                        pointer++;
                        previousResult = localresult;
                        localresult += result[pointer];
                    }

                    if (ResultDict.TryGetValue((pointerPos.x, pointerPos.y, localresult), out var val))
                    {
                        i = pointer - 1;
                        presses += val.res;
                        pointerPos = (val.x, val.y);
                        continue;
                    }
                ...
                ResultDict[(pointerStart.x, pointerStart.y, result)] = (presses, pointerPos.x, pointerPos.y);

                if (this.wrapped != null)
                {
                    var res = this.wrapped.GetPresses(presses);
                    //ResultDict[(pointerStart.x, pointerStart.y, result)] = (res, pointerPos.x, pointerPos.y);
                    return res;
                }
                else
                {
